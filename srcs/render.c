...
void	set_door_visibility(t_ray *ray, t_img *texture, double tex_x_f)
{
...
void	calculate_door_texture(t_ray *ray, t_img *texture)
{
	...
	easedopen_width = ease_in_out_cubic(door->open_width);
	tex_x_f = ray->wall_x + easedopen_width;
	if (door->open_width < 0.9)
		set_door_visibility(ray, texture, tex_x_f);
	else
