...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2025/11/05 15:10:05 by aaladeok          #+#    #+#             */
/*   Updated: 2025/11/05 15:10:09 by aaladeok         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
/*
	This function loads door frames to be used in doors animation.
	It does so by generating a filename: dynamically and
	saving all loaded frames in frames array"
*/
...
static int	get_frame(t_data *data, int i, int width, int height)
{
	...
	snprintf(path, sizeof(path), "./assets/door/Layer-1_sprite_%02d.xpm", i
		+ 1);
	data->textures.door_arr.frames[i].img = mlx_xpm_file_to_image(data->mlx,
			path, &width, &height);
